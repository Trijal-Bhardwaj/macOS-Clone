...
setInterval(function () {
  ...
  homePageTitleDateTime.innerText = `${day} , ${hrs} : ${min} : ${sec} ${amOrPm}`;
}, 1000);
  ...
  currDockIcon.addEventListener("dblclick", function (e) {
    ...
    if (attributeCheck) {
      if (
        ...
        attributeCheck == "z-index: 80; display: none;" ||
        attributeCheck == "display: none; z-index: 80;"
      ) {
...
function cancelCircle(currDockIcon, appBox) {
  ...
  let cancelCircle = cancelCircleBox.querySelector(
    ".mac-dialogBox-title-icon1"
  );
  ...
  cancelCircle.addEventListener("click", function () {
    ...
    setTimeout(() => {
      ...
      currDockIcon.removeAttribute("style");
      let divDot = document.querySelector(
        `.active-dot[appbox-dot="${checkBoxDot}-dot"]`
      );
      divDot.parentNode.removeChild(divDot);
      return;
    }, 220);
...
function fullScreenCircle(appBox) {
  ...
  let fullScreenCircle = fullScreenCircleBox.querySelector(
    ".mac-dialogBox-title-icon3"
  );
  fullScreenCircle.addEventListener("click", function () {
    ...
    if (
      document.querySelector(appBox).getAttribute("style") &&
      document
        .querySelector(appBox)
        .getAttribute("style")
...
function minimize(appBox) {
  ...
  let minimize = minimizeBox.querySelector(".mac-dialogBox-title-icon2");
